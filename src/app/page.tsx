...
import { apiFetchPublic } from '@/components/scribia/api-fetch'
    ...
    try {
      if (mode === 'login') {
        ...
        const data = await apiFetchPublic('/api/auth/register', {
          method: 'POST',
          body: JSON.stringify({ email, password, name, role, teacherCode: teacherCode || undefined }),
        })
        login(data.user, data.token)
    ...
    <Card className="w-full max-w-md mx-auto">
      <CardHeader className="text-center">
        <div className="flex justify-center mb-2">
          <div className="flex size-12 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-600 to-teal-500 shadow-lg">
            <Feather className="size-6 text-white" />
          </div>
        </div>
        <CardTitle className="text-xl">
          {mode === 'login' ? 'Accedi a ScribIA' : 'Crea il tuo Account'}
        </CardTitle>
        <CardDescription>
          {mode === 'login'
            ? 'Inserisci le tue credenziali per continuare'
            : 'Registrati per iniziare a scrivere in italiano'}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === 'register' && (
            <>
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button
                    ...
                    className={role === 'STUDENT' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}
                  >
                    ✍️ Studente
                  </Button>
                  <Button
                    ...
                    className={role === 'TEACHER' ? 'bg-amber-600 hover:bg-amber-700' : ''}
                  >
                    📚 Insegnante
                  </Button>
                </div>
              ...
              {role === 'STUDENT' && (
                <div className="space-y-2">
                  <Label htmlFor="teacherCode">Codice Insegnante (opzionale)</Label>
                  <Input
                    id="teacherCode"
                    value={teacherCode}
  ...
  return (
    <div className="min-h-screen flex flex-col">
            ...
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              ...
              <SheetContent side="right" className="w-72">
                ...
                <div className="flex flex-col gap-3 px-2">
                  <Button variant="outline" className="w-full justify-center" onClick={() => { setMobileOpen(false); openAuth('login') }}>
                    Accedi
                  </Button>
                  <Button className="w-full justify-center bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-700 hover:to-teal-600 text-white" onClick={() => { setMobileOpen(false); openAuth('register') }}>
                    Inizia Gratis
                    <ArrowRight className="size-4 ml-1" />
                  </Button>
                </div>
      ...
      <main className="flex-1">
        ...
        {/* Pricing */}
        <AnimatedSection id="prezzi" className="py-20 md:py-28 bg-muted/30">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <motion.div variants={fadeUp} custom={0} className="text-center mb-16">
              <Badge variant="secondary" className="mb-4 text-xs font-medium tracking-wide uppercase">
                Prezzi
...
export default function Home() {
  ...
  // Show AuthInitializer if there's a token but not yet authenticated
